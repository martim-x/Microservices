...
async def test_register_service_success(
    ...
    mock_service_token_repository.db_create_service_token.return_value = (
        SimpleNamespace(
            ...
            allowed_scopes=json.dumps(["orders.read", "orders.write"]),
            is_active=True,
    ...
    assert result.allowed_scopes == ["orders.read", "orders.write"]
...
async def test_issue_service_access_token_success(
    ...
    assert payload.sub == "order-service"
    assert payload.aud == "user-product-service"
    assert payload.scope == ["orders.read", "orders.write"]
...
def test_external_service_error() -> None:
    exc = ExternalServiceError("external error")
    assert exc.status_code == 502
    assert exc.detail == "external error"


def test_external_service_unavailable_error() -> None:
    exc = ExternalServiceUnavailableError("external unavailable")
    assert exc.status_code == 503
    assert exc.detail == "external unavailable"
