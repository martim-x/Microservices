...
def build_app(health_state=None) -> FastAPI:
    app = FastAPI()
    ...
    [
        {
            ...
            "shutdown_requested": False,
            "checks": {"mq": {"required": True, "ok": True}},
        },
        {
            "startup_complete": True,
        ...
        },
        {
            "startup_complete": True,
            "shutdown_requested": False,
            "checks": {"mq": {"required": True, "ok": False}},
        },
    ],
)
...
def test_ready_success() -> None:
    ...
    assert data["checks"]["mq"]["ok"] is True


def test_ready_failed_dependency() -> None:
    client = TestClient(
        build_app(
            {
                "startup_complete": True,
